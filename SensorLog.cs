...
namespace BandSensorLogToTcx
{
	...
	//=========================================================================================================
	{
		...
		//------------------------------------------------------------------------------------------------------
		public static bool IsSensorLog( in  Stream stream, 
								...
								if( currentSequence != null )
								{
									...
									currentSequence.Waypoints.Add(
										new Waypoint()
										{
											...
											ElevationFromMeanSeaLevel = rawElevation > 0 ? rawElevation / 100 : 0,
		...
		//------------------------------------------------------------------------------------------------------
		public List<Workout> CreateWorkouts( ExportType type = ExportType.TCX | ExportType.HeartRate |
											 ExportType.Cadence )
		//------------------------------------------------------------------------------------------------------
		{
					...
					foreach( var workoutMarker in sequence.WorkoutMarkers )
					{
						if( workoutMarker.Action == DistanceAnnotationType.Start )
						{
							if( currentWorkout != null )
							{
								...
								AddWorkoutData( ref currentWorkout, in heartRates, in elevationList, in positionList, in galvanicList, in temperatureList, in stepsList );
						...
						foreach( var wayPoint in sequence.Waypoints )
						{
							if( wayPoint.TimeStamp >= currentWorkout.LastSplitTime - TimeSpan.FromSeconds( 10 ) )
							{
								if( ( wayPoint.EstimatedHorizontalError <= iExactThreshold || wayPoint.EstimatedVerticalError <= iExactThreshold ) &&
									( ( currentWorkout.Type != EventType.Running && currentWorkout.Type != EventType.Biking ) || wayPoint.ElevationFromMeanSeaLevel > 0 ) )
								{
		...
		public bool AddWorkoutData( ref Workout currenWorkout,
									in Dictionary<System.DateTime, int> heartRateList,
									in Dictionary<System.DateTime, double> elevationList,
									in Dictionary<System.DateTime, GpsPosition> positionList,
									in Dictionary<System.DateTime, uint> galvanicList,
									in Dictionary<System.DateTime, double> temperatureList,
									in Dictionary<System.DateTime, uint> stepsList )
		//------------------------------------------------------------------------------------------------------
		{
			uint lastGalvanic = 0;
			uint lastSteps = 0;
				...
				foreach( var workout in Workouts )
				{
					if( ( workout.Type == EventType.Running || workout.Type == EventType.Hike || workout.Type == EventType.Sleeping || workout.Type == EventType.Biking ) && workout.TrackPoints.Count > 0 )
					{
						...
						if( workout.Summary != null )
						{
							...
							else if( workout.Summary.HFAverage < 158 )
								strWorkoutType = "Hard";
							else
						...
						foreach( var trackPoint in workout.TrackPoints )
						{
								...
								( type & ExportType.GalvanicSkinResponse ) == ExportType.GalvanicSkinResponse )
							{
								...
								tcxDatabase.Activities.Activity[0].Lap[0].Track[iIndex].Extensions.Any[0] =
									doc.CreateElement( "TPX", "http://www.garmin.com/xmlschemas/ActivityExtension/v2" );
								if( ( type & ExportType.GalvanicSkinResponse ) == ExportType.GalvanicSkinResponse )
								{
									XmlElement elem = doc.CreateElement( "Galvanic", "http://www.garmin.com/xmlschemas/ActivityExtension/v2" );
								...
								if( ( type & ExportType.Temperature ) == ExportType.Temperature )
								{
									XmlElement elem = doc.CreateElement( "Temp", "http://www.garmin.com/xmlschemas/ActivityExtension/v2" );
